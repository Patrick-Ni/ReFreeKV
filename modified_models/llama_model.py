...
from .evict_utils import init_pyramidkv, init_snapkv, init_H2O, init_StreamingLLM, init_ATTN
...
def llama_attn_forward_H2O(
    ...
    if past_key_value is not None:
        ...
        if key_states.shape[-2] == kv_seq_len:
            ...
            self.layer_prefill_time = elapsed_time
            past_key_value.update(key_states_compress, value_states_compress, self.layer_idx, cache_kwargs)
        else:
            # self.kv_seq_len += q_len
            key_states, value_states = past_key_value.update(key_states, value_states, self.layer_idx, cache_kwargs)
...
def llama_attn_forward_StreamingLLM(
    ...
    if past_key_value is not None:
        ...
        if key_states.shape[-2] == kv_seq_len:
            ...
            elapsed_time = end_time - start_time  # 计算时间差
            # print(f"prefilling time: {elapsed_time} seconds")
            self.layer_prefill_time = elapsed_time
            past_key_value.update(key_states_compress, value_states_compress, self.layer_idx, cache_kwargs)
        else:
            # self.kv_seq_len += q_len
    ...
    if not output_attentions:
        ...
        attn_weights = torch.matmul(query_states, key_states_compress.transpose(2, 3)) / math.sqrt(self.head_dim)
        attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32).to(query_states.dtype)
        attn_weights = nn.functional.dropout(attn_weights, p=self.attention_dropout, training=self.training)
    return attn_output, attn_weights, past_key_value
...
def prepare_inputs_for_modified_generation_llama(
        self, input_ids, past_key_values=None, attention_mask=None, inputs_embeds=None, **kwargs
):
    # if past_key_values is None:
    #     for layer in self.model.layers:
